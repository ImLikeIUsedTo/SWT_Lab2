                            ...
                            <table class="table table-bordered" width="100%" cellspacing="0">
                                ...
                                <c:forEach var="user" items="${users}">
                                    <tr>
                                        ...
                                        <td>
                                            <a class="btn btn-sm btn-primary" href="AdminManageUserServlet?action=edit&id=${user.userId}">Edit</a>
                                            <a class="btn btn-sm btn-danger" href="AdminManageUserServlet?action=delete&id=${user.userId}" onclick="return confirm('Bạn chắc chắn muốn xóa?');">Delete</a>
                                            <c:choose>
                                                <c:when test="${user.locked}">
                                                    <a class="btn btn-sm btn-warning" href="AdminManageUserServlet?action=unlock&id=${user.userId}">Mở khóa</a>
                                                </c:when>
                                                <c:otherwise>
                                                    <a class="btn btn-sm btn-secondary" href="AdminManageUserServlet?action=lock&id=${user.userId}">Khóa</a>
                                                </c:otherwise>
                                            </c:choose>
                                        </td>
                                    </tr>
